...
# REPLACE the entire synthetic_objective function WITH:
def synthetic_objective(x: Tensor) -> Tensor:
    """
    DTLZ2 function with M=3, k=5, x shape [N,7], outputs [N,3].
    Domain: x_i ∈ [0,1].
    """
    M = 3
    k = 5
    
    # Split variables: first M-1=2 variables are for position, last k=5 for distance
    x_m = x[:, :M-1]  # First 2 variables [x1, x2]
    x_k = x[:, M-1:]  # Last 5 variables [x3, x4, x5, x6, x7]
    
    # Calculate g(x_M) = sum((x_j - 0.5)^2) for j=M to n
    ...
    for i in range(M):
        ...
        if i == 0:  # f1: product of cos(x_j * pi/2) for j=1 to M-1
            prod = torch.prod(torch.cos(x_m * torch.pi / 2), dim=1)
        elif i == M-1:  # f_M: sin(x_1 * pi/2)
            prod = torch.sin(x_m[:, 0] * torch.pi / 2)
        else:  # f_i: cos(x_1 * pi/2) * ... * cos(x_{M-i} * pi/2) * sin(x_{M-i+1} * pi/2)
            cos_prod = torch.prod(torch.cos(x_m[:, :M-1-i] * torch.pi / 2), dim=1)
            sin_term = torch.sin(x_m[:, M-1-i] * torch.pi / 2)
            prod = cos_prod * sin_term
        
        # f_i = (1 + g(x_M)) * product_term
        f_i = (1 + g) * prod
...
def plot_objective_outputs(objective_function, input_ranges, resolution=30):
    ...
    Args:
        ...
        resolution (int): Number of points per input dimension.
    """
    n_inputs = len(input_ranges)
    if n_inputs not in (3, 4, 7):  # Added 7 to supported dimensions
        raise ValueError("Supports only 3, 4, or 7 input dimensions for grid-based plotting.")

    # For 7D, we'll use random sampling instead of full grid due to memory constraints
    if n_inputs == 7:
        # Use random sampling for 7D case
        n_samples = resolution**3  # Reduce total samples for memory efficiency
        grid = torch.zeros(n_samples, n_inputs)
        for i, (lo, hi) in enumerate(input_ranges):
            grid[:, i] = torch.rand(n_samples) * (hi - lo) + lo
    else:
        # build torch.linspace for each dimension
        grids = [torch.linspace(lo, hi, resolution) for lo, hi in input_ranges]

        # meshgrid and stack into (N, n_inputs)
        if n_inputs == 3:
    ...
    # HV
    ref_point = torch.zeros(n_outputs) - 5
...
plot_objective_outputs(
    synthetic_objective,
    input_ranges=[(0,1) for _ in range(7)],  # Changed to 7 dimensions with [0,1] range
    resolution=50
)
...
def get_candidate_d(acq_function, train_x_scaled,xs_scaled):
    ...
    candidate, _ = optimize_acqf_SA(
        ...
        choices=choices,
        unique=True,
        num_iter=4000,  # You can adjust these parameters
        T_init=5.0,
        alpha=0.95
    )
...
def plot_pareto_comparison(benchmark_results):
    ...
    grid = torch.stack([X.flatten(), Y.flatten(), Z.flatten()], dim=-1)  # shape (n_grid**3, 3)
