...
# --- at the top, change input dim and candidate sampling -------------------



# original:
# xs = torch.rand(n_candidates, num_input)
# remains, now with 3‐D samples:
xs = torch.rand(n_candidates, num_input).to(device)


# --- replace synthetic_objective with Kursawe --------------------------------
from torch import Tensor

# Replace the entire Kursawe function with:
def synthetic_objective(x: Tensor) -> Tensor:
    """
    ZDT1 function, x shape [N,n], outputs [N,2].
    Domain: x_i ∈ [0,1].
    """
...
# --- when plotting or sampling grids, update input_ranges to 3 dims ------------
...
def plot_objective_outputs(objective_function, input_ranges, resolution=30):
    ...
    Args:
        ...
        resolution (int): Number of points per input dimension for low-dim, total points for high-dim.
    """
    n_inputs = len(input_ranges)
    
    # For high-dimensional problems, use random sampling instead of grid
    if n_inputs > 4:
        # Random sampling for high-dimensional spaces
        n_samples = resolution * 1000  # Use more samples for better coverage
        grid = torch.zeros(n_samples, n_inputs)
        
        for i, (lo, hi) in enumerate(input_ranges):
            grid[:, i] = torch.rand(n_samples) * (hi - lo) + lo
            
    elif n_inputs in (3, 4):
        # Grid-based sampling for low-dimensional spaces
        grids = [torch.linspace(lo, hi, resolution) for lo, hi in input_ranges]
        
        if n_inputs == 3:
            X1, X2, X3 = torch.meshgrid(*grids, indexing='ij')
            grid = torch.stack([X1.flatten(), X2.flatten(), X3.flatten()], dim=-1)
        else:  # n_inputs == 4
            X1, X2, X3, X4 = torch.meshgrid(*grids, indexing='ij')
            grid = torch.stack([X1.flatten(), X2.flatten(), X3.flatten(), X4.flatten()], dim=-1)
    
    elif n_inputs == 2:
        # Grid-based sampling for 2D
        grids = [torch.linspace(lo, hi, resolution) for lo, hi in input_ranges]
        X1, X2 = torch.meshgrid(*grids, indexing='ij')
        grid = torch.stack([X1.flatten(), X2.flatten()], dim=-1)
        
    elif n_inputs == 1:
        # 1D case
        grid = torch.linspace(input_ranges[0][0], input_ranges[0][1], resolution).unsqueeze(-1)
        
    else:
        raise ValueError(f"Unsupported number of input dimensions: {n_inputs}")

    # evaluate
    objectives = objective_function(grid)  # shape (N, n_outputs)
    n_outputs = objectives.shape[1]
    ...
    for i in range(n_outputs):
        for j in range(n_outputs):
            ax = axs[i, j]
            if i == j:
                ax.hist(objectives[:, i].cpu().numpy(), bins=50)
                ax.set_title(f'Histogram of f{i+1}')
            else:
                ax.scatter(objectives[:, j].cpu().numpy(),
                           objectives[:, i].cpu().numpy(),
                           s=1, alpha=0.5)
                ax.set_xlabel(f'f{j+1}')
                ax.set_ylabel(f'f{i+1}')

    plt.tight_layout()
    plt.savefig("objective_space.jpg", dpi=900)
    plt.show()

    # HV - Set appropriate reference point based on objective function
    # For ZDT1, outputs are typically in [0,1] range, so ref_point should be below 0
    ref_point = torch.zeros(n_outputs) - 5  # Slightly below zero for ZDT1
    
    train_y = objectives[:, :n_outputs]
    pareto_y = train_y[is_non_dominated(train_y)]
    hv = Hypervolume(ref_point).compute(pareto_y)
    print("____Hypervolume_________", hv)
    print(f"Number of points evaluated: {len(objectives)}")
    print(f"Number of Pareto points: {len(pareto_y)}")
    print(f"Objective ranges: f1=[{train_y[:, 0].min():.3f}, {train_y[:, 0].max():.3f}], f2=[{train_y[:, 1].min():.3f}, {train_y[:, 1].max():.3f}]")


plot_objective_outputs(
    synthetic_objective,
    input_ranges=[(0,1)] * num_input,  # 30 dimensions of [0,1]
    resolution=20
)
...
train_x = torch.rand(initial_samples, num_input)
...
class DGPLastLayer3(gpytorch.models.ApproximateGP):
    def __init__(self, input_dims, output_dims, num_inducing=128, linear_mean=True):
        ...
        self.mean_module = LinearMean(input_dims) #gpytorch.means.ConstantMean(batch_shape=torch.Size([10]))
...
class DGPHiddenLayer(DeepGPLayer):
    def __init__(self, input_dims, output_dims, num_inducing=32, linear_mean=True):
        ...
        self.mean_module =  LinearMean(input_dims) #ConstantMean(batch_shape=batch_shape) if linear_mean else
...
def train_model_with_validation(train_x, train_y, val_x, val_y, num_tasks, train_yvar=None, test_yvar=None, num_epochs=3000, reduction=8):
    ...
    model.gp_model = None #standard_gp_model.to(device)
...
from botorch.optim.optimize import _filter_invalid, _filter_infeasible, _split_batch_eval_acqf

def optimize_acqf_SA(
    ...
    T_init: float = 1.0,
    alpha: float = 0.99,
    p_change: list[float] = [0.6, 0.3, 0.1],
) -> tuple[Tensor, Tensor]:
    r"""Optimize over a discrete set of points using simulated annealing.
    ...
    Args:
        ...
        num_iter: Number of iterations for simulated annealing.
        T_init: Initial temperature for simulated annealing.
        alpha: Cooling rate for temperature in simulated annealing.
        p_change: Probability distribution of changing 1, 2, or q points.
    ...
    # Filter invalid and infeasible choices
    ...
    # For q=1, we can just evaluate all candidates and pick the best one
    if q == 1:
    ...
    # Initialize with random choices for q > 1
    device = choices.device
    dtype = choices.dtype

    # Randomly initialize current batch
    indices = torch.randperm(len(choices), device=device)[:q]
    current_batch = choices[indices].clone()

    # Evaluate initial batch
    with torch.no_grad():
        current_acq = acq_function(current_batch)
    try:
        current_acq=current_acq.mean()
    except:
        current_acq=current_acq
    best_batch = current_batch.clone()
    best_acq = current_acq.clone()

    # Set up temperature
    T = T_init

    # Define the number of points to change probabilities
    # This determines how many points in the batch we change at each iteration
    max_change = min(3, q)  # Limit max changes to q
    if max_change == 1:
        p_change = [1.0]
    elif max_change == 2:
        p_change = [0.7, 0.3]
    else:
        p_change = p_change[:max_change]
        p_change = [p / sum(p_change) for p in p_change]  # Normalize

    # Begin simulated annealing
    base_X_pending = acq_function.X_pending

    for i in range(num_iter):
        # Create a new candidate batch by perturbing current batch
        new_batch = current_batch.clone()

        # Decide how many points to modify (1, 2, or more)
        num_changes = np.random.choice(range(1, max_change + 1), p=p_change)
        num_changes = min(num_changes, q)  # Ensure we don't try to change more points than batch size

        # Randomly select indices to change
        change_indices = torch.randperm(q, device=device)[:num_changes]

        # Select new points from the choices
        candidate_indices = torch.randperm(len(choices), device=device)[:num_changes]
        new_points = choices[candidate_indices]

        # Insert new points
        for i, idx in enumerate(change_indices):
            new_batch[idx] = new_points[i]

        # Evaluate new batch
        if unique:
            # Check if there are duplicates in the new batch
            if len(torch.unique(new_batch, dim=0)) < q:
                # Skip this iteration if there are duplicates
                continue

        # Update X_pending for acquisition function if needed
        acq_function.set_X_pending(None)

        # Compute new acquisition value
        with torch.no_grad():
            new_acq = acq_function(new_batch)
        try:
            new_acq=new_acq.mean()
        except:
            new_acq=new_acq
        # Acceptance probability

        delta_val = (new_acq - current_acq)
        # print(new_acq,"_________New_Acq_____-")
        # print(current_acq,"_________Current_Acq_____-")
        # print(delta_val,"_________delta_val_____-")
        exponent = (delta_val / T).clamp(min=-100, max=100)
        accept_prob = torch.exp(exponent)
        if delta_val > 0 or torch.rand(1, device=device).item() < accept_prob:
            current_batch = new_batch.clone()
            current_acq = new_acq.clone()
    ...
    # Reset acq_function to original X_pending state
    acq_function.set_X_pending(base_X_pending)

    return best_batch, best_acq
...
def get_acquisition_d(model, train_y_scaled, ref_point):
    ...
    train_y_scaled2=train_y_scaled[:,:num_obj]
...
def get_candidate_d(acq_function, train_x_scaled,xs_scaled):
    ...
    mask = ~torch.any((xs_scaled.unsqueeze(1) == train_x_scaled.unsqueeze(0)).all(-1), dim=1)
    choices = xs_scaled[mask]
    # print(choices,"_________test______________")
    # Use the new simulated annealing optimization
    candidate, _ = optimize_acqf_SA(
        acq_function,
        q=batch_size,
        choices=choices,
        unique=True,
        num_iter=10000,  # You can adjust these parameters
        T_init=2.0,
        alpha=0.98
    )

    print("______________Optimization SA______________________")
    return candidate
...
def fit_gp(train_x, train_y):
    ...
    return model
def fit_dgp(train_x, train_y, num_epochs=500, reduction=1):
    """
...
def run_benchmark(surrogate_funcs, surrogate_names, num_queries, init_points):
    ...
    initial_train_x =  torch.rand(init_points, num_input).to(device) #init_points
    ...
    #xs = torch.rand(n_candidates, num_input).to(device)
    xs =  torch.rand(50000,num_input ).to(device) 
...
def plot_pareto_comparison(benchmark_results):
    ...
    """
    # Generate points for objective space background using random sampling for high-dimensional input
    n_samples = 5000  # Use random sampling instead of grid for 30D space
    grid = torch.rand(n_samples, num_input)  # Random points in [0,1]^30 for ZDT1
...
surrogate_funcs = [fit_gp,fit_dgp]
surrogate_names = ["GP","DeepGP"]  # Will rename to DGP later
