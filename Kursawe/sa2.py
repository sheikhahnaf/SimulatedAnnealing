...
if input_space == 'discrete':
    xs = (2*torch.rand(n_candidates, num_input).to(device) - 1) * 5
else:
    bounds = torch.stack([torch.ones(num_input)*0.1, torch.ones(num_input)]).to(device)
    xs = None  # not used
...
def synthetic_objective(x: Tensor) -> Tensor:
    """
    Kursawe function, x shape [N,3], outputs [N,2].
    Domain: x_i ∈ [-5,5].
    """
    # ensure x is in correct domain if you want to rescale; here assume samples in [-5,5]
    x1, x2, x3 = x.unbind(-1)
    # first objective
    f1 = -10 * torch.exp(-0.2 * torch.sqrt(x1**2 + x2**2)) \
         -10 * torch.exp(-0.2 * torch.sqrt(x2**2 + x3**2))
    # second objective
    f2 = torch.abs(x1)**0.8 + 5*torch.sin(x1**3) \
         + torch.abs(x2)**0.8 + 5*torch.sin(x2**3) \
         + torch.abs(x3)**0.8 + 5*torch.sin(x3**3)
    return torch.stack([f1, f2], dim=-1)
...
def plot_objective_outputs(objective_function, input_ranges, resolution=30):
    ...
    if n_inputs not in (3, 4):
        raise ValueError("Supports only 3 or 4 input dimensions for grid-based plotting.")
...
train_x = (2*torch.rand(initial_samples, 3) - 1) * 5
...
def get_candidate_d(acq_function, train_x_scaled,xs_scaled):
    ...
    device='cuda' if torch.cuda.is_available() else 'cpu'
    bounds  = torch.stack([torch.ones(num_input)*(-5),torch.ones(num_input)*5]).to(device)
...
def run_benchmark(surrogate_funcs, surrogate_names, num_queries, init_points):
    ...
    initial_train_x =  (2*torch.rand(init_points, 3).to(device) - 1) * 5 #init_points
